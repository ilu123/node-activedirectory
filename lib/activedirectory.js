...
defaultAttributes = originalDefaultAttributes = {
  user: [ 
    ...
    'comment', 'description', 'givenName'
  ],
  group: [
    'dn', 'cn', 'description', 'uniqueMember'
  ]
};
...
function getUserQueryFilter(username) {
  ...
  if (! username) return('(objectClass=organizationalPerson)');
  if (isDistinguishedName.call(self, username)) {
    return('(&(objectClass=organizationalPerson)(cn='+parseDistinguishedName(username)+'))');
  }

  return('(&(objectClass=organizationalPerson)(|(sAMAccountName='+username+')(userPrincipalName='+username+')))');
}
...
function getGroupQueryFilter(groupName) {
  ...
  if (! groupName) return('(objectClass=groupOfUniqueNames)');
  if (isDistinguishedName.call(self, groupName)) {
    return('(&(objectClass=groupOfUniqueNames)(distinguishedName='+parseDistinguishedName(groupName)+'))');
  }
  return('(&(objectClass=groupOfUniqueNames)(cn='+groupName+'))');
}
...
function isGroupResult(item) {
  ...
  if ((item.objectClass) && (item.objectClass.length > 0)) {
    return(_.any(item.objectClass, function(c) { return(c.toLowerCase() === 'groupOfUniqueNames'); }));
  }
  return(false);
}
...
function isUserResult(item) {
  ...
  if ((item.objectClass) && (item.objectClass.length > 0)) {
    return(_.any(item.objectClass, function(c) { return(c.toLowerCase() === 'organizationalPerson'); }));
  }
  return(false);
}
...
ActiveDirectory.prototype.getUsersForGroup = function getUsersForGroup(opts, groupName, callback) {
  ...
  self.findGroup(_.defaults({}, _.omit(opts || {}, 'attributes'), {
    ...
    if (typeof(group.uniqueMember) === 'string') {
      group.uniqueMember = [ group.uniqueMember ];
    }
    users = []
    async.forEach(group.uniqueMember, function(e, asyncCallback) {
      self.findUser(opts, e.split("=")[1], false, function(err, user) {
        if (err) {
          asyncCallback(err);
          return;
        }
...
ActiveDirectory.prototype.findGroups = function findGroup(opts, callback) {
  var self = this;
  var defaultGroupFilter = '(objectClass=groupOfUniqueNames)(!(objectClass=computer))(!(objectClass=user))(!(objectClass=person))';
...
ActiveDirectory.prototype.findUsers = function findUsers(opts, includeMembership, callback) {
  var self = this;
  var defaultUserFilter = '(|(objectClass=inetOrgPerson)(objectClass=person))(!(objectClass=computer))(!(objectClass=group))';
