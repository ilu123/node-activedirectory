...
function getUserQueryFilter(username) {
  ...
  if (! username) return('(objectClass=inetOrgPerson)');
  if (isDistinguishedName.call(self, username)) {
    return('(&(objectClass=inetOrgPerson)(distinguishedName='+parseDistinguishedName(username)+'))');
  }

  return('(&(objectClass=inetOrgPerson)(|(sAMAccountName='+username+')(userPrincipalName='+username+')))');
}
...
function isUserResult(item) {
  ...
  if ((item.objectClass) && (item.objectClass.length > 0)) {
    return(_.any(item.objectClass, function(c) { return(c.toLowerCase() === 'inetOrgPerson'); }));
  }
  return(false);
}
...
ActiveDirectory.prototype.getUsersForGroup = function getUsersForGroup(opts, groupName, callback) {
  ...
  self.findGroup(_.defaults({}, _.omit(opts || {}, 'attributes'), {
    ...
    async.forEach(group.uniqueMember, function(e, asyncCallback) {
      self.findUser(opts, e, false, function(err, user) {
        if (err) {
          asyncCallback(err);
          return;
        }
